...
def words():
    for tree in conll.read_trees_conll(sys.stdin):
...
def nth(num):
    trees_read = 0
    for i, tree in enumerate(conll.read_trees_conll(sys.stdin)):
        if i + 1 == num:
            conll.write_tree_conll(sys.stdout, tree)
...
def shuf():
    trees = list(conll.read_trees_conll(sys.stdin))
...
def _html(limit, fields, file):
    ...
    """
    tree_to_html.write_prologue_html(file)
    ...
    for i, tree in enumerate(conll.read_trees_conll(sys.stdin)):
        ...
        # Draw.
        static = i >= _HL_LIMIT
        tree_to_html.write_tree_html(file, tree, fields, [], static)

    tree_to_html.write_epilogue_html(file)
...
def _grep_text(pattern):
    ...
    """
    # Parse pattern.
    pattern = tree_script.parse_pattern(pattern)
    ...
    for tree in conll.read_trees_conll(sys.stdin):
        ...
        # Print.
        if match:
            conll.write_tree_conll(UTF8Writer(sys.stdout), tree)
...
def _grep_html(pattern, limit, fields, file):
    ...
    for tree in conll.read_trees_conll(sys.stdin):
        ...
        if matches:
            tree_to_html.write_tree_html(file, tree, fields, matches, static)
            printed += 1
...
def _gdb_tree(scripts, scripts_text, tree, fields, file):
    ...
    tree_to_html.write_tree_html(file, tree, fields)
    ...
    backrefs_map = {}
    state = tree_state.TreeState(tree, backrefs_map)
    exc = None
    ...
    for script_no, script in enumerate(scripts):
        ...
        while True:
            ...
            tree_to_html.write_tree_html(file, tree, fields, [node])
            ...
            for action_no, action in enumerate(script.actions):
                ...
                try:
                    action.apply(state)
                except tree_action.TreeActionError as e:
                    exc = e
...
def _gdb(scripts_filename, fields, file):
    ...
    with open(scripts_filename, 'rt') as f:
        ...
        scripts = tree_script.parse_scripts(scripts_text)
...
if __name__ == '__main__':
   ...
   # words_p.add_argument('FILE', help='scripts file')
    ...
    #head_p.add_argument('FILE', help='scripts file')
    ...
    #tail_p.add_argument('FILE', help='scripts file')

    # Shuffle.
    shuf_p = subparsers.add_parser('shuf', help='shuffle trees')
    #shuf_p.add_argument('FILE', help='scripts file')
